...
class AuthService {
  async registerWithEmail(userData: RegistrationData): Promise<UserProfile> {
    try {
      ...
      const emailSnapshot = await get(ref(db, 'users'));
      ...
      const verificationRef = ref(db, `verificationQueue/${userCredential.user.uid}`);
      ...
      // Save login history
      const loginHistoryRef = ref(db, `loginHistory/${userCredential.user.uid}/${Date.now()}`);
      ...
      const preferencesRef = ref(db, `userPreferences/${userCredential.user.uid}`);
  ...
  async registerWithGoogle(): Promise<UserProfile> {
    try {
      ...
      if (userSnapshot.exists()) {
        // Update last login
        const loginHistoryRef = ref(db, `loginHistory/${result.user.uid}/${Date.now()}`);
        await set(loginHistoryRef, {
          timestamp: serverTimestamp(),
          type: 'login',
          method: 'google',
      ...
      const userProfile: UserProfile = {
        ...
        verificationStatus: result.user.emailVerified ? 'verified' : 'pending',
        createdAt: Date.now(),
        updatedAt: Date.now(),
        role: 'user',
        settings: {
      ...
      // Create user preferences
      const preferencesRef = ref(db, `userPreferences/${result.user.uid}`);
      await set(preferencesRef, {
        theme: 'system',
        language: 'en',
        notifications: {
